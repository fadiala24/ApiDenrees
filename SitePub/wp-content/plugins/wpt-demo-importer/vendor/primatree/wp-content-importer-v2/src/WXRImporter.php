<?php
namespace PrimaTree\WPContentImporter2;
...
class WXRImporter extends \WP_Importer {
	...
	public function __construct( $options = array() ) {
		...
		//update 1.1 'update_attachment_guids'   => false was true
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				//update 1.1 problem here
				//if(($key!="_elementor_data")&&(wp_slash( $key )!=_elementor_data))
	...
	protected function post_process_comments( $todo ) {
		foreach ( $todo as $comment_id => $_ ) {
			...
			// Run the update
			//update 1.1 $data['comment_ID'] = $comment_ID;
	...
	protected function replace_attachment_urls_in_content() {
		...
		foreach ( $this->url_remap as $from_url => $to_url ) {
			...
			//update 1.1 //$result = $wpdb->query( $query );
			$wpdb->query( $query );	
		}
	}
	...
	protected function post_exists( $data ) {
		...
		if ( $this->options['prefill_existing_posts'] ) {
			// PADEPRE: fix for custom post types. The guids in the prefilled section are escaped, so these ones should be as well.
