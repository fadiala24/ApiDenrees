<?php
/**
 * Class for declaring the content importer used in the WPT Demo Importer plugin
 *
 * @package PADEPRE
 */
...
class Importer {
	...
	/**
	 * The instance of the PADEPRE\Logger class.
	 *
	 * @var object
	 */
	public $logger;
	...
	/**
	 * The instance of the WPT Demo Importer class.
	 *
	 * @var object
	 */
	private $PADEPRE;
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		// Get the PADEPRE (main plugin class) instance.
	...
	public function import_content( $import_file_path ) {
		...
		if ( strpos( ini_get( 'disable_functions' ), 'set_time_limit' ) === false ) {
			set_time_limit( apply_filters( 'pt-PADEPRE/set_time_limit_for_demo_data_import', 300 ) );
		}
		...
		if ( ! apply_filters( 'pt-PADEPRE/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-PADEPRE/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			if ( ! empty( $message ) ) {
				$this->PADEPRE->append_to_frontend_error_messages( $message );
			}

			// Add message to log file.
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!' , 'pt-PADEPRE' ) . PHP_EOL . $message,
				$this->PADEPRE->get_log_file_path(),
				''
			);
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->PADEPRE->get_current_importer_data(), $this->get_importer_data() );

		Helpers::set_PADEPRE_import_data_transient( $data );
	}
}
