<?php
/**
 * Class for the widget importer used in the WPT Demo Importer plugin.
 *
...
class WidgetImporter {
	...
	public static function import( $widget_import_file_path ) {
		$results       = array();
		$PADEPRE          = PrimaAppDemoImport::get_instance();
		$log_file_path = $PADEPRE->get_log_file_path();
		...
		if ( is_wp_error( $results ) ) {
			...
			// Add any error messages to the frontend_error_messages variable in PADEPRE main class.
			$PADEPRE->append_to_frontend_error_messages( $error_message );

			// Write error to log file.
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing widgets', 'pt-PADEPRE' )
			);
		}
		else {
			ob_start();
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file could not be found.', 'pt-PADEPRE' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import data could not be read. Please try a different file.', 'pt-PADEPRE' )
			);
		}

		// Hook before import.
		do_action( 'pt-PADEPRE/widget_importer_before_widgets_import' );
		$data = apply_filters( 'pt-PADEPRE/before_widgets_import_data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'Sidebar does not exist in theme (moving widget to Inactive)', 'pt-PADEPRE' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', 'pt-PADEPRE' ); // Explain why widget not imported.
				}

				// Filter to modify settings object before conversion to array and import.
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below).
				// Ideally the newer wie_widget_settings_array below will be used instead of this.
				$widget = apply_filters( 'pt-PADEPRE/widget_settings', $widget ); // Object.
				...
				$widget = apply_filters( 'pt-PADEPRE/widget_settings_array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = __( 'Widget already exists', 'pt-PADEPRE' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					do_action( 'pt-PADEPRE/widget_importer_after_single_widget_import', $after_widget_import );

					// Success message.
					if ( $sidebar_available ) {
						$widget_message_type = 'success';
						$widget_message      = __( 'Imported', 'pt-PADEPRE' );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Imported to Inactive', 'pt-PADEPRE' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'pt-PADEPRE' ); // Show "No Title" if widget instance is untitled.
		...
		// Hook after import.
		do_action( 'pt-PADEPRE/widget_importer_after_widgets_import' );

		// Return results.
		return apply_filters( 'pt-PADEPRE/widget_import_results', $results );
	}
	...
	private static function available_widgets() {
		...
		return apply_filters( 'pt-PADEPRE/available_widgets', $available_widgets );
	}
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'pt-PADEPRE' );
		}
